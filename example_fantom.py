...
from hidden_utils.fantom_eval_utils import FantomEvalAgent  # type: ignore
import typer
import time
...
def load_fantom_data() -> pd.DataFrame:
...
def analyze_perceivers(df: pd.DataFrame) -> List[str]:
    ...
    for perceptions in df["gt_perception"]:
        try:
            ...
            logging.error(f"Error parsing gt_perception: {e}")
            continue
    return sorted(list(all_perceivers))


def get_perception_stats(df: pd.DataFrame) -> Dict[str, Any]:
...
def load_saved_results(model_name: str) -> List[Dict[str, Any]]:
...
async def run(
    ...
    print("Distribution of samples per set_id:")
...
async def run_single_experiment_fantom(
    ...
    Returns:
        Result dictionary with experiment data and model response
    """
...
async def run_batch(
    rows: pd.DataFrame, save: bool = False, model_name: str = "gpt-4-mini"
) -> List[Dict[str, Any]]:
    """Run a batch of FANToM experiments."""
    logging.info(f"Running batch with model {model_name}")
    tasks = [
        run_single_experiment_fantom(row, model_name, save)
        for _, row in rows.iterrows()
    ]
    results = await asyncio.gather(*tasks)
    fantom_eval_agent = FantomEvalAgent(model_name)
    evaluated_output = fantom_eval_agent.evaluate_response(
        results, [result["answer"] for result in results]
    )
    return cast(List[Dict[str, Any]], evaluated_output)
