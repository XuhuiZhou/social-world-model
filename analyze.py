...
def analyze_simulation_files(directory):
    # Get the directory path
    dir_path = Path(directory).resolve()  # Get absolute path
    
    # Statistics counters
    stats = defaultdict(lambda: {'total': 0, 'incorrect': 0})
    mismatched_files = []
    ...
    with open(csv_path, 'w', newline='') as csvfile:
        fieldnames = ['filename', 'question_type', 'question', 'correct_answer', 'given_answer']
