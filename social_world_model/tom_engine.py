...
class ToMEngine:
    ...
    async def initialize_simulation_from_socialized_context(self, socialized_context: dict[str, Any]) -> None:
    ...
    async def socialize_context(self, context: str, example_analysis: str = "",  feedback: str = "") -> SocializedContext:
        """
    ...
    async def critique_and_improve_context(
        ...
        while attempts < max_attempts:
            ...
            # Get critic feedback
            is_good, critique = await self.critic_socialize_context(context_json, example_patterns)
            if is_good:
        ...
        print(f"Reached maximum attempts ({max_attempts}). Returning the last context version.")
        return current_context

    async def critic_socialize_context(self, socialized_context: str, example_patterns: str) -> Tuple[bool, str]:
        """
        ...
        format_instructions = PydanticOutputParser(pydantic_object=SocializedContext).get_format_instructions()
        ...
        if evaluation.startswith("yes"):
            ...
            return False, f"Previous attempt: {socialized_context}\n\nFeedback: {evaluation}"
