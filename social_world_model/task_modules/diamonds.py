...
def create_diamonds_result(
    ...
    if true_ans == "NA":
        ...
        accuracy = 1 if math.isclose(true_ans, pred_ans, rel_tol=2e-2) else 0
...
def diamonds_evaluation_report(results: list[dict[str, Any]]) -> None:
    ...
    for result in results:
        dataset_type = result.get("id", "").split("_")[0] if "_" in result.get("id", "") else "base"
    ...
    for dataset_type, counts in dataset_types.items():
        print(f"  {dataset_type}: {counts['correct']/counts['total']:.4f} ({counts['correct']}/{counts['total']})")
