...
async def run_tomi_experiments(batch_size: int = 10, save_simulation: bool = False) -> None:
    ...
    correct_count = 0
    # Run experiments for each scenario in the dataset
    for idx, row in tomi_data.iterrows():
    ...
    You need to first reason about the question (majorly focusing where the object has been moved to, and focus on the most detailed position possible e.g., the object is in A and A is in B, then you should focus on 'A') and then answer the question with the following format:<reasoning>(reasoning)</reasoning> <answer>(answer)</answer>",
        ...
        # Optimize for multi-order ToM
        if row['char2'] and str(row['char2'])!="nan":
            # Instead of initializing the actual char2's memory, we initialize the char1's memory of what char2's memory should be
        ...
        print(f"Initialized simulation for {row['story_with_perceivers']}")
        if row['char2'] and str(row['char2'])!="nan":
            # The question is about char1's belief of char2's belief
        ...
        if save_simulation:
            ...
            with open(f"data/simulations/{row['index']}.json", "w") as f:
...
if __name__ == "__main__":
    ...
    asyncio.run(run_tomi_experiments(save_simulation=True))