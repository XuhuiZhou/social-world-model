...
BenchmarkType = Literal[
    "tomi", "fantom", "confaide", "cobra_frames", "hitom", "flawfictions"
]
...
class ToMBenchmarkRunner:
    ...
    async def _run_vanilla(
        ...
        if benchmark_type == "tomi":
            ...
            parsed_result = self._parse_response(response, row)
            result = create_flawfictions_result(parsed_result, row)
        return result
    ...
    async def _run_socialized_context(
        ...
        elif benchmark_type in ["hitom", "flawfictions"]:
            context = row["story"]
        else:
            context = row["context"]
        ...
        if (
            ...
            if row["set_id"] in engine.existing_socialized_contexts:
                ...
                if benchmark_type == "flawfictions":
    ...
    async def _run_simulation(
        ...
        if benchmark_type == "tomi":
            ...
            parsed_result = await flawfictions_simulation(row, engine)
            result = create_flawfictions_result(parsed_result, row)
        else:
            result = await self._run_vanilla(row, benchmark_type)
        if not result:
...
def run_benchmark(
    benchmark_type: str = typer.Argument(
        ...,
        help="Type of benchmark to run (tomi/fantom/confaide/hitom/flawfictions)",
    ...
    if dataset_path is None:
        dataset_path = {
            ...
            "flawfictions": "./data/flawfictions_data/flawed_fictions100.json",
        }[benchmark_type]
    ...
    if mode == "generate_socialized_context":
        ...
        if benchmark_type in ["fantom", "confaide", "hitom", "flawfictions"]:
...
async def _run_benchmark(
    ...
    if benchmark_type == "tomi":
        ...
        flawfictions_evaluation_report(all_results)
